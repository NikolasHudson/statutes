"""End-to-end Iowa Code ingest from a probe-JSON file.

    python manage.py ingest_iowa_code path/to/iowa_code_probe.json
    python manage.py ingest_iowa_code path/to/iowa_code_probe.json --dry-run
"""
...
class Command(BaseCommand):
    help = "Ingest an Iowa Code probe JSON into the corpus tables."
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=str,
            default=None,
            help="ISO date for the new versions. Defaults to today.",
        )
        parser.add_argument(
            "--source-kind",
            type=str,
    ...
    def handle(self, *args, **opts):
        ...
        self.stdout.write(
            ...
            f"(code year {parsed.code_year})."
        )
        if parsed.skipped_sections:
            self.stdout.write(
                self.style.WARNING(
                    f"Skipped {len(parsed.skipped_sections)} malformed section(s) "
                    f"from RTF scrape (chapter/number mismatches):"
                )
            )
            for s in parsed.skipped_sections:
                self.stdout.write(
                    f"  - chapter={s.chapter} number={s.number!r} heading={s.heading[:50]!r}"
                )
        ...
        raw = persist_raw_input(
            ...
            notes=f"ingest_iowa_code from {json_path.name}",
        )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Ingest complete. Run #{run.pk} pending review."
            )
        )
