"""Surface-agnostic corpus tool implementations.

These are the *pure* direct-lookup tool bodies — they take JSON-able inputs,
call into the corpus service layer (``apps.corpus.services.lookups``), and
return JSON-able dicts. They live in ``apps.corpus`` (not ``apps.mcp_server``)
so that BOTH the human-facing chat surface (``apps.api.chat``) and the
agent-facing MCP surface (``apps.mcp_server.tools``) import them from here.
This removes the old backwards dependency where ``apps.api`` imported from
``apps.mcp_server``.
...
seen the metadata that says when it was current.

Ambiguity rule: when a citation doesn't resolve unambiguously we return
the candidate list. We never silently substitute.

The search/retrieval pipeline (hybrid search + rerank + assembly) lives in
``apps.corpus.services.retrieval`` (``retrieve_context``); the thin MCP/chat
search adapters are in ``apps.mcp_server.tools`` / ``apps.api.chat``.
"""
...
def _node_dict(node: Node) -> dict[str, Any]:
    ...
    # Research notes injected into the tool result so the model reads them
    # WITH the authority, on every surface: attorney construction/scope
    # guidance and act-supersession warnings (a signed session law repealed
    # this section after our Code edition's text). Supersession outranks
    # construction when both exist — currency beats nuance. Rejected notes
    # never surface. One indexed query per node; hits are rare.
    notes = list(
        node.research_notes.filter(kind__in=("act_supersession", "construction"))
        .exclude(review_status="rejected")
        .order_by("-checked_at")
    )
    if notes:
        note = next(
            (n for n in notes if n.kind == "act_supersession"), notes[0]
        )
...
def _caselaw_decision(node: Node) -> Node:
    """The decision (cluster) node carrying the case name + reporter citations.

    A vector/chunk hit rolls up to an *opinion* node whose heading is just
    "Opinion" and whose metadata has neither the case name nor the cites — its
    parent is the decision. A direct decision hit is its own decision."""
    if node.parent_id is not None and node.parent is not None:
        return node.parent
    return node


def _annotate_caselaw(node: Node, d: dict[str, Any]) -> None:
    """Replace the generic citation/heading for a caselaw node with the case
    name + reporter cite (e.g. "State v. Farrell, 223 N.W.2d 270"), so the
    chat "Sources" footer shows the case instead of the raw cluster path
    ("Iowa cl-cluster-10012997"). The case name and cites live on the decision
    node, not the opinion, so resolve to it first. Also exposes ``case_id`` (the
    decision node id) for the /cases/<id> reader link."""
    decision = _caselaw_decision(node)
    md = decision.source_metadata or {}
    name = md.get("case_name") or decision.heading or node.heading or "Case"
    # Prefer an official reporter cite over a LEXIS/Westlaw vendor cite.
    cites = [c for c in (md.get("citations") or []) if c]
    reporter = next(
        (c for c in cites if "LEXIS" not in c and " WL " not in c),
        cites[0] if cites else "",
    )
    court = md.get("court_name") or ""
    year = (md.get("date_filed") or "")[:4]
    d["citation"] = f"{name}, {reporter}" if reporter else name
    d["heading"] = ", ".join(p for p in (court, year) if p)
    d["case_id"] = decision.id
