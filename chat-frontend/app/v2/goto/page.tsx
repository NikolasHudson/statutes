...
// resolves that pair through /api/browse/resolve and lands on the v2 reader,
		...
		browseResolve(source, cite)
			.then((r) => {
				...
				if (r.found) {
					router.replace(
						r.is_chapter
							? `/v2/chapter/${r.node_id}`
							: `/v2/section/${r.node_id}`,
					);
				} else if (r.candidates.length > 0) {
					router.replace(`/v2/section/${r.candidates[0].node_id}`);
				} else {
	...
	return (
		<div className="px-5 py-10 sm:px-8">
			{error ? (
				...
				>
					{error}{" "}
					<Link href="/v2" className="text-[var(--cds-link)] hover:underline">
						Back to the Library.
					</Link>
				</Notification>
			) : (
