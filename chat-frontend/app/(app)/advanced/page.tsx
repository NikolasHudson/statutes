...
// Advanced search — fielded query builder over the filters the search
// endpoint actually supports (content type, court, status, decided-year
// range). Builds the same /results URL the results rail produces, so the
...
export default function V2AdvancedPage() {
	...
	const run = () => {
		const q = query.trim();
		if (!q) return;
		router.push(`/results?${buildSearchQuery(q, toSearchFilters(filters))}`);
	};
