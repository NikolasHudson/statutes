...
// Shared Carbon (IBM design system) shell + primitives for the app mockup
// suite under /app-carbon-mockup. Design exploration only — nothing here is
// wired to the live app. Token cheatsheet lives in docs/carbon-design-system.md;
// the first screen built on this language is /browse-carbon-mockup.
//
// One markup tree serves both Carbon themes ("white" and "g100"): tokens are
// CSS custom properties applied on the root wrapper by CarbonRoot (the route
...
// ---------------------------------------------------------------------------
// Theme context — CarbonRoot wraps the whole route group (see layout.tsx), so
// the sun/moon toggle carries across screens without remounting.
// ---------------------------------------------------------------------------
...
export function CarbonRoot({ children }: { children: React.ReactNode }) {
	// Case text is the workhorse surface, so the app suite defaults to the
	// white theme (the browse mockup demoed g100); the header toggle is global.
...
// ---------------------------------------------------------------------------
// UI shell — g100-dark header (both themes) + side nav linking every screen
// ---------------------------------------------------------------------------

const NAV: {
	group: string;
	items: { href: string; label: string; icon: LucideIcon; detail?: string }[];
}[] = [
	{
		group: "Workspace",
		items: [
...
export function ShellHeader({
	...
	return (
		<header className="flex h-12 shrink-0 items-center border-[#393939] border-b bg-[#161616] text-white">
			...
			<Link href="/app-carbon-mockup" className="text-sm hover:underline">
			...
			<div className="flex-1">
				{NAV.map((g) => (
					<div key={g.group}>
						...
						{g.items.map((it) => {
							const isActive = it.href.endsWith(active);
							const Icon = it.icon;
							return (
								<Link
									key={it.href}
