...
const COURT_CITE: Record<string, string> = {
  iowa: "Iowa",
  iowactapp: "Iowa Ct. App.",
};

type Section = { id: string; label: string; depth: number };

// ---------------------------------------------------------------------------
// Opinion text formatting
//
...
function opinionHeadings(op: CaseOpinion): string[] {
  if (op.body_segments) {
    const out: string[] = [];
...
function parseOpinion(text: string): Block[] {
  const lines = text
    .split("\n")
    .map((l) => l.trim())
    .filter(Boolean);
...
function renderBlock(b: Block, headingId?: string): ReactNode {
  ...
  switch (b.kind) {
    ...
    case "heading":
      return (
        <h3 id={headingId} className="mt-7 mb-3 scroll-mt-4 font-bold text-[1.05em]">
          {renderInline(prefix + b.text)}
        </h3>
      );
    case "label":
    ...
    default:
      return (
        <p className="mb-3.5 leading-relaxed">{renderInline(prefix + b.text)}</p>
      );
  }
}
      ...
      {segments.map((b, i) => {
        ...
        if (b.k === "fn") {
          return (
            ...
            >
              {b.mark ? <sup className="mr-1 font-semibold">{b.mark}</sup> : null}
              {renderRuns(b.runs)}
            </p>
          );
        }
...
function buildCitation(d: CaseDetail): string {
...
function DetailsList({ details }: { details: { label: string; value: string }[] }) {
  return (
...
export function CaseConsole({ data }: { data: CaseDetail }) {
  ...
  return (
    <div className="flex h-dvh flex-col print:h-auto">
      <header className="flex items-center gap-2 border-b bg-background px-3 py-2 print:hidden">
        ...
        <span className="truncate font-medium text-sm">{data.case_name}</span>
          ...
          <div className="hidden w-60 shrink-0 overflow-y-auto border-r p-4 lg:block print:hidden">
            {hasOutline ? (
              <nav aria-label="Document outline">
                ...
                <OutlineList sections={sections} active={active} onJump={jump} />
              </nav>
            ) : null}
            {details.length > 0 ? (
            ...
            {hasLeftRail ? (
              <details className="mt-4 rounded-md border lg:hidden">
                ...
                <div className="space-y-4 border-t px-3 py-3">
                  ...
                  {details.length > 0 ? <DetailsList details={details} /> : null}
                </div>
              </details>
            ) : null}

            <div className="mt-6 text-foreground" style={{ fontSize: `${readingPx}px` }}>
              {data.head_matter ? (
              ...
              {data.opinions.map((op) => (
                <section key={op.id} id={`op-${op.id}`} className="mt-10 scroll-mt-4">
                  <SectionHeadingLabel>
                    {op.heading}
                    {op.per_curiam ? (
