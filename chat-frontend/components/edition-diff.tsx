...
export function EditionDiff({
  ...
  const [selected, setSelected] = useState<number | null>(initialNodeId ?? null);
  ...
  useEffect(() => {
    ...
    // onSection is a stable setter from the parent; excluded to avoid refetch.
...
function EditionPicker({
  ...
  return (
    <div className="flex items-center gap-3 border-b px-4 py-3 text-sm">
      ...
      {coverageNote ? (
        <span className="ml-auto text-xs text-muted-foreground">{coverageNote}</span>
      ) : null}
    </div>
  );
}
